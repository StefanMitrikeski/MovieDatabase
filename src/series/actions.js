...
function getSingleSeries(name) {
  const getSeriesByNameQuery = 'SELECT * FROM series where title = ? OR language=? OR length=?';
  return new Promise((resolve, reject) => {
    con.query(getSeriesByNameQuery, [name,name,name],(err, results) => {
      if (err) reject (err);
      resolve(results);
    });
  });
...
const getSeriesByName = async (req, res, next) => {
  ...
  try {
    const seriesName = await getSingleSeries(name);
